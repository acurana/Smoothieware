...
void Mpx595::UpdateConfig(uint16_t module_checksum, uint16_t name_checksum)
{
    ...
    data_s1.set(0);
    load_s2.set(1);     // "/CS" to high initially
}
...
{
    for (int i=channels-1; i >= 0; --i) // @todo check generated code
    {
        data_s1.set(data & (0x01 << i)); // highest bit is shifted in 1st
        clock_s0.set(1); // clock next data bit to data pin
        clock_s0.set(0);
    }
    load_s2.set(0);     // for a '595 we only need a pulse
    load_s2.set(1);     // rising edge will latch data
}
