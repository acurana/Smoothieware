...
void Robot::load_config()
{
    ...
    for (size_t a = 0; a < MAX_ROBOT_ACTUATORS; a++) {
        ...
        Pin pins_ms[3]; // m1, m2, m3 for fabbster
        for (size_t i = 0; i < 3; ++i) { // offset 6 in array ACTUATOR_CHECKSUMS
            pins_ms[i].from_string(THEKERNEL->config->value(motor_checksums[a][i+6])->by_default("nc")->as_string())->as_output();
        }

        StepperMotor *sm = new StepperMotor(pins[0], pins[1], pins[2],
                pins_ms[0], pins_ms[1], pins_ms[2]);
        ...
        actuators[a]->change_microsteps(THEKERNEL->config->value(motor_checksums[a][9])->by_default(16)->as_int());
    }
