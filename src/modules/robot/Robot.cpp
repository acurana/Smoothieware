...
void Robot::load_config()
{
    ...
    for (size_t a = X_AXIS; a <= Z_AXIS; a++) {
        ...
        for (size_t i = 0; i < 3; ++i) { // offset 6 in array ACTUATOR_CHECKSUMS
            pins_ms[i].from_string(THEKERNEL->config->value(checksums[a][i+6])->by_default("nc")->as_string())->as_output();
        }
        ...
        actuators[a]->change_microsteps(THEKERNEL->config->value(checksums[a][9])->by_default(16)->as_int());
    }
