...
void Extruder::config_load()
{
	...
	Pin m1_pin, m2_pin, m3_pin;
	m1_pin.from_string(   THEKERNEL->config->value(extruder_checksum, this->identifier, m1_pin_checksum            )->by_default("nc" )->as_string())->as_output();
	m2_pin.from_string(   THEKERNEL->config->value(extruder_checksum, this->identifier, m2_pin_checksum            )->by_default("nc" )->as_string())->as_output();
    m3_pin.from_string(   THEKERNEL->config->value(extruder_checksum, this->identifier, m3_pin_checksum            )->by_default("nc" )->as_string())->as_output();
    ...
    stepper_motor = new StepperMotor(step_pin, dir_pin, en_pin, m1_pin, m2_pin, m3_pin);
    ...
    stepper_motor->change_microsteps(THEKERNEL->config->value(extruder_checksum, this->identifier, microsteps_checksum)->by_default(16)->as_int());
}
