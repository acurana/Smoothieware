...
Kernel::Kernel(){
    ...
    switch( __mriPlatform_CommUartIndex() ) {
        case 0:
#ifdef COMPILE_FOR_FABBSTER
            this->serial = new(AHB0) SerialConsole(P4_28, P4_29, this->config->value(uart0_checksum,baud_rate_setting_checksum)->by_default(DEFAULT_SERIAL_BAUD_RATE)->as_number());
#else
